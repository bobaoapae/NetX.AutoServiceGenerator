                    ...
                    foreach (var member in autoServiceServerConsumerInterface.GetMembers())
                    {
                        if (member is IMethodSymbol methodSymbol)
                        {
                            ...
                            foreach (var parameterSymbol in methodSymbol.Parameters)
                            {
                                ...
                                {
                                    writeParameters.Append('\t', 4).AppendLine($"{autoServiceServerConsumerInterface.Name}_{methodSymbol.Name}_stream.ExWrite({(nameSymbol.EnumUnderlyingType != null ? $"({nameSymbol.EnumUnderlyingType})" : "") + parameterSymbol.Name});");
                                }
                                else
                                {
                                    writeParameters.Append('\t', 4).AppendLine($"{autoServiceServerConsumerInterface.Name}_{methodSymbol.Name}_stream.ExWrite({parameterSymbol.Name});");
                                }

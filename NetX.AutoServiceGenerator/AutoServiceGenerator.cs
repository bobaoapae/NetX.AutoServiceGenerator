...
namespace NetX.AutoServiceGenerator
{
    ...
    public class AutoServiceGenerator : ISourceGenerator
    {
        ...
        public void Execute(GeneratorExecutionContext context)
        {
            ...
            if (autoServiceConsumerAttributeDefinition == null || autoServiceProviderAttributeDefinition == null || autoServiceClientManagerInterfaceDefinition == null || autoServiceServerManagerInterfaceDefinition == null)
            {
...

            {
                ...
                foreach (var autoServiceServerManager in autoServiceServerManagers)
                {
                    ...
                    var allImplementedServices = autoServiceServerManager.GetAttributes().Where(data => data.AttributeClass?.Name == autoServiceProviderAttributeDefinition.Name).Select(data => ((INamedTypeSymbol)data.ConstructorArguments[0].Value)).ToList();
                    ...
                    foreach (var autoServiceClientConsumerInterface in autoServiceClientConsumerInterfaces)
                    {
                        ...
                        autoServicesClientConsumerInitializations.Append('\t', 2).AppendLine($"{autoServiceClientConsumerInterface.Name.Substring(1)} = new {autoServiceClientConsumerInterface.Name.Substring(1)}{autoServiceServerManagerName}ClientConsumer(this, streamManager);");
                        ...
                        foreach (var member in autoServiceClientConsumerInterface.GetMembers())
                        {
                            if (member is IMethodSymbol methodSymbol)
                            {
                                ...
                                serviceImplementations
                                    .Append('\t', 1)
                                    .AppendLine(string.Format(methodReturnTypeGeneric != null ? autoServiceClientConsumerMethodResource : autoServiceClientConsumerMethodResourceVoid, autoServiceClientConsumerInterface.Name, methodSymbol.Name, methodReturnType, methodReturnTypeGeneric, parameters,
                        ...
                        foreach (var member in interfaceServer.GetMembers())
                        {
                            if (member is IMethodSymbol methodSymbol)
                            {
                                ...
                                autoServiceServerProcessorProxies
                                    .Append('\t')
                                    .AppendLine(string.Format(methodReturnGenericType != null ? autoServiceServerProcessorMethodProxyResource : autoServiceServerProcessorMethodProxyResourceVoid, implementedServerService.Name, interfaceServer.Name, methodCode, methodSymbol.Name,
...

            {
                ...
                foreach (var autoServiceClientManager in autoServiceClientManagers)
                {
                    ...
                    foreach (var autoServiceServerConsumerInterface in autoServiceServerConsumerInterfaces)
                    {
                        ...
                        autoServicesServerConsumerInitializations.Append('\t', 2).AppendLine($"{autoServiceServerConsumerInterface.Name.Substring(1)} = new {autoServiceServerConsumerInterface.Name.Substring(1)}{autoServiceClientManagerName}ServerConsumer(_netXClient, manager);");

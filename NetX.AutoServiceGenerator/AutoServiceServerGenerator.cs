    ...
    public static void Generate(Compilation compilation, ImmutableArray<ClassDeclarationSyntax> classes, SourceProductionContext context)
    {
        ...
        foreach (var autoServiceServerManager in autoServiceServerManagers)
        {
            ...
            foreach (var autoServiceClientConsumerInterface in autoServiceClientConsumerInterfaces)
            {
                ...
                autoServicesClientConsumerInitializations.Append('\t', 2).AppendLine($"{autoServiceClientConsumerInterface.Name.Substring(1)} = new {autoServiceClientConsumerInterface.Name.Substring(1)}{autoServiceServerManagerName}ClientConsumer(this, _logger, streamManager);");
